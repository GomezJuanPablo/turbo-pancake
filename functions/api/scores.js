...
export async function onRequest(context) {
  ...
  // ── PUT (temporary seed) ───────────────────────────────────────────
  // Replaces a board wholesale. Gated by a one-off key; removed after seeding.
  if (method === 'PUT') {
    let body;
    try { body = await request.json(); }
    catch { return respond({ error: 'Invalid JSON body' }, 400); }
    if (!body || body.key !== 'seed_yxOgkDWn9hxuQnHegCDFTQEq2qfQ6ggd') {
      return respond({ error: 'Unauthorized' }, 401);
    }
    const examUpper = (body.exam ?? '').toUpperCase();
    if (!VALID_EXAMS.has(examUpper)) return respond({ error: 'Unknown exam' }, 400);
    if (!Array.isArray(body.scores)) return respond({ error: 'scores must be an array' }, 400);
    const board = [];
    for (const r of body.scores) {
      ...
      board.push({
        ...
        exam: examUpper,
        mode: typeof r.mode === 'string' ? r.mode.slice(0, 40) : 'Exam Sim',
        score, total,
        pct: Math.round((score / total) * 100),
